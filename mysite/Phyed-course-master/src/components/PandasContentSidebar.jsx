...
import PandasList from "./PandasData";

const PandasContentSidebar = ({ children }) => {
  ...
  return (
    <div>
      <section className="h-12 top-0 sticky w-full z-10 bg-blue-600 text-white dark:bg-blue-900 flex items-center">
        <BiMenu
          className="text-4xl ml-4 md:hidden"
      ...
      <div className="flex h-fit">
        ...
        >
          <h4 className="py-4 text-xl px-2 font-semibold">Pandas Tutorial</h4>
          {PandasList.map((item, id) => {
            return (
              <NavLink
                to={item.path}
                key={id}
                className="flex w-64 items-center my-2 py-1.5 pl-2 hover:bg-gray-250"
...
export default PandasContentSidebar;
