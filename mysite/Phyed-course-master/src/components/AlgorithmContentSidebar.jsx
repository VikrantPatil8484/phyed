...
import algorithmlist from './algorithmData';
import { BiMenu } from 'react-icons/bi'
import CoursesList from './CoursesList';
import SubMenu from './SubMenu';

const AlgorithmContentSidebar = ({ children }) => {
  ...
  return (
    <div>
         <section className='h-12 top-0 sticky w-full z-10 bg-blue-600 text-white dark:bg-blue-900 flex items-center' >
            < BiMenu className='text-4xl ml-8 md:hidden' onClick={() => setOpen(!open)} />
            < CoursesList />
        </section>
        <div className='flex h-full'> 
            ...
            >
                    <h4 className='py-2 text-xl px-2'>Algorithms Tutorial</h4>
                { 
                    algorithmlist.map((item, id) => {  
                        if(item.subRoutes) {
                            return <SubMenu item={item} key={item.name} setOpen={setOpen} />
                        }
                        return (
                            (
                                <NavLink to={item.path} key={id} className='flex w-64 items-center my-2 py-1.5 pl-2  hover:text-blue-400 hover:bg-white'  activeclassName="active">
...
export default AlgorithmContentSidebar;